...
export class ProductGenerationService {
    ...
    public DataHistory: string = environment.baseUrl + "v1/DataHistory/";
    // Data
    ...
    exportList(field: string): Observable<any> {
    var HTTPOptions = {
      'responseType': 'blob' as 'json'
    }
    return this.http.get<any>(this.url + "filter_table_testing/"+ "?" + field, HTTPOptions);
  }
    ...
    //chart new 
    getImeiSerialChart() {
      let urlSerial = this.url + 'product_registration_chart'
      return this.http.get<any>(urlSerial).pipe(
        tap((res)=>{
          //this.ProductRegistration = res
        })
    ...
    DataAudit(body): Observable<History> {
      return this.http.post<History>(this.DataHistory, body).pipe(
        tap((res) => {
        })
      );
    }
